...
'use strict';
  ...
  nacl.secretbox.open = function(box, nonce, key) {
    ...
    var c = new Buffer(crypto_secretbox_BOXZEROBYTES + box.length);
    var m = new Buffer(c.length);
    for (var i = 0; i < box.length; i++) c[i+crypto_secretbox_BOXZEROBYTES] = box[i];
    if (c.length < 32) return false;

    if (tweetnacl.crypto_secretbox_open(m, c, c.length, nonce, key) !== 0) return false;
  ...
  nacl.scalarMult = function(n, p) {
    ...
    var q = new Buffer(crypto_scalarmult_BYTES);
